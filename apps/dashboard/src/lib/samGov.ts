 ...
 * Requires VITE_SAM_GOV_API_KEY to be set in environment variables.
 * 
...
export async function searchOpportunities(params: SamSearchParams = {}): Promise<{ opportunities: SamOpportunity[]; totalCount: number }> {
    // @ts-ignore - Handle both Vite and Node environments
    const apiKey = (typeof process !== 'undefined' ? process.env.SAM_GOV_API_KEY : null) || 
                   // @ts-ignore
                   (typeof import.meta !== 'undefined' && import.meta.env ? import.meta.env.VITE_SAM_GOV_API_KEY : null);
