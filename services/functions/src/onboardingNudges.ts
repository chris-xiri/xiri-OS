...
interface NudgeConfig {
    key: string;              // flag field on company doc
    minHoursAfterSignup: number;
    requiresNoBids: boolean;  // true = only send if 0 bids
    requiresBidsNoProposal: boolean; // true = has bids but no proposal sent
    subject: string;
    bodyHtml: (name: string, extra?: Record<string, any>) => string;
}
...
export const sendOnboardingNudges = onSchedule(
    {
    ...
    async () => {
        ...
        for (const companyDoc of companiesSnap.docs) {
            ...
            if (subStatus === "active" || subStatus === "past_due") continue;
            ...
            const hasSentProposal = bidsSnap.docs.some(
                (b) => b.data().sentAt || b.data().status === "sent"
            );

            // Evaluate each nudge
            for (const nudge of NUDGES) {
                // Already sent?
                if (company[nudge.key]) continue;

                // Too early?
                if (hoursSinceSignup < nudge.minHoursAfterSignup) continue;

                // Check bid conditions
                if (nudge.requiresNoBids && hasBids) continue;
                if (nudge.requiresBidsNoProposal && (!hasBids || hasSentProposal)) continue;

                // For the 5-day inactive nudge, skip if they've created bids recently
                if (nudge.key === "nudge5dInactiveSent" && hasSentProposal) continue;
